...
print('\n#1')
square = [x ** 2 for x in range(20 + 1)];
print(f"Squares: \n{square}\n")


# 2. Calculate the first 50 power of two. Use power_of_two as the name of the list.
print('\n#2')
power_of_two = [2 ** x for x in range(50 + 1)];
print(f"Power of two: \n{power_of_two}\n")


# 3. Calculate the square root of the first 100 numbers. Use sqrt as the name of the list.
print('\n#3')
sqrt = [sqrt(x) for x in range(100 + 1)];
print(f"Sqrt: \n{sqrt}\n")
...
print('\n#5')
odds = [x for x in range(1, 100 + 1) if x % 2 != 0];
print(f"Odds: \n{odds}\n")
...
print('\n#7')
nonVowels = ''.join([a for a in teststring if a not in vowels]);
print(f'String w vowels : \t{teststring}\nStrings w/o vowels: \t{nonVowels}\n')
...
print('\n#8')
capital_letters = re.findall('[A-Z]', teststring_2);
print(f'String: \t{teststring_2}\nAll caps in string: \t{capital_letters}\n')


# 9. Find all the consonants in the sentence 'The quick brown fox jumped over the lazy dog'.
print('\n#9')
consonants = re.findall('[a-z]', teststring_2);
...
folders = [f for f in listdir('../../../m1/') if f.endswith('')];
print(f'The folders in m1 are: \t{folders}\n')
...
try:
    if userNum_1.isdigit() and userNum_2.isdigit():
        ...
        if int(userNum_2) != 0:
            ...
            raise ZeroDivisionError('Zero is not a valid input. Try again!')
    else:
        raise TypeError
...
try:
    ...
    print('No linux platform was found')
finally:
    print('All Done.\n')
...
def sqr(num):
    while not isinstance(num, int):
        try:
            ...
            result = int(num)**2
            return print(f'The function result is {result}. \nAll Done!')
...
for div in range(2,10):
    for num in range(1,201):
        # Only done for numbers from 1-200 for readability
        if num % div == 0:
            nums.append(num)
        else:
            pass
...
class NumBelowThreshold(ValueError):
    def __init__(self, input):
        self.message = print(f'The input {input} is below the defined threshold')
    def __str__(self):
        return self.message


def MeanPerSection(marks, num_sections):
    return marks / num_sections
...
try:
    ...
    if Num_of_Sections > 2:
        result = MeanPerSection(Total_Marks, Num_of_Sections)
        print(f'the result is {result}')
    else:
        raise NumBelowThreshold(input=Num_of_Sections) from ValueError
